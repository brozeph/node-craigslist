...
module.exports = (function (self) {
	...
	*/
	function getRequestOptions (options, query) {
		if (typeof query === 'undefined' && options) {
			query = options;
			options = {};
		}
		...
		requestOptions.hostname =
			(options.city || self.options.city || defaultOptions.city) +
			baseHost;
	...
	*/
	self.search = function (options, query, callback) {
		...
		var
			listingHandler = new htmlParser.DefaultHandler(function (err, dom) {
				...
				listingsDOM.forEach(function (listingDOM) {
					...
					listing.url = titleDOM.attribs['href'] || '';
			...
			requestOptions = getRequestOptions(options, query);
		...
		if (options.minAsk) {
			requestOptions.path += minAsk + options.minAsk;
		}

		if (options.maxAsk) {
			requestOptions.path += maxAsk + options.maxAsk;
		}
